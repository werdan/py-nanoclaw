...
For reminders and scheduled tasks, ALWAYS use the scheduler MCP tools (schedule_task, list_tasks, pause_task, delete_task). Do NOT use any built-in cron, task, or remote agent features.
After creating a task, confirm with the task ID, cron expression, and next run time.
